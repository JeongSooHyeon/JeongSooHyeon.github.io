...
- 함수의 이름은 함수의 시작 주소
- 함수 시그니처(signature : 함수의 반환 타입과 매개변수 리스트)와 같게 선언
...
정적 함수 포인터는 함수 시그니처만 알면 쉽게 선언 가능
정적 함수인 전역 함수, namespace 내의 전역 함수, static 멤버 함수 모두 함수 호출 규약이 같아서 함수 포인터가 같다.
...
- 정적 함수는 모두 같은 함수 포인터 pf를 사용. pf 는 정적 함수 포인터
...
멤버 함수 포인터는 함수 포인터 선언에 어떤 클래스의 멤버 함수를 가리킬 것인지 클래스 이름을 지정해야 함
...
연산자 우선순위로 인해 객체와 .*, ->* 연산자 사이에 <u>() 연산자를 사용</u>
